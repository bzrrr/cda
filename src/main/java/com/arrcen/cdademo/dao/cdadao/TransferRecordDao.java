...
public interface TransferRecordDao extends JpaRepository<TransferRecord,String> {

    @Query(value = "select * from PUB13_转科记录 where 住院号 = ?1",nativeQuery = true)
    TransferRecord findBy住院号(String 住院号);
    ...
    @Query(value = "SELECT 代码名称 from PUB_GB中医病证分类与代码 WHERE 值域名称 = '中医病证分类与代码表(GB/T 15657-1995)' and 代码值 = ?1",nativeQuery = true)
	String find入院诊断_中医证候名称By入院诊断_中医证候代码(String 入院诊断_中医证候编码);
